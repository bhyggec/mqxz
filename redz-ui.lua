...
local Funcs = {} do
	...
	function Funcs:ToggleParent(Obj, Parent, Bool)
		if Bool ~= nil then
			...
			Obj.Parent = Obj.Parent ~= Parent and Parent or nil
		end
	end
...
local Connections, Connection = {}, redzlib.Connection do
	...
	function Connection:FireConnection(CoName, ...)
		local TargetConnection = type(CoName) == "string" and Connections[CoName] or Connections[CoName.Name]
		if TargetConnection then
			for _,Func in pairs(TargetConnection) do
				task.spawn(Func, ...)
			end
		end
	end
...
local GetFlag, SetFlag, CheckFlag do
	...
	Connection.FlagsChanged:Connect(function(Flag, Value)
		local ScriptFile = redzlib.Settings.ScriptFile
		if not db and ScriptFile and writefile then
			...
			if Success then
				local SaveSuccess = pcall(writefile, ScriptFile, Encoded)
				if SaveSuccess then
...
local WindowUIScale = ViewportSize.Y / 450
local ButtonUIScale = ViewportSize.Y / 450
...
local WindowLayer = Create("Frame", ScreenGui, {
	Name = "WindowLayer",
	BackgroundTransparency = 1,
	Size = UDim2.fromScale(1,1)
})

local ButtonLayer = Create("Frame", ScreenGui, {
	Name = "ButtonLayer",
	BackgroundTransparency = 1,
	Size = UDim2.fromScale(1,1)
})

local WindowScale = Create("UIScale", WindowLayer, {
	Name = "WindowScale",
	Scale = WindowUIScale
})

local ButtonScale = Create("UIScale", ButtonLayer, {
	Name = "ButtonScale",
	Scale = ButtonUIScale
})
...
local function GetWindowScale()
	return WindowScale.Scale
end

local function GetButtonScale()
	return ButtonScale.Scale
end
...
-- 拖拽功能函数（修复：动态获取当前缩放）
local function MakeDrag(Instance, GetScale)
    GetScale = GetScale or GetWindowScale
    ...
    task.spawn(function()
        ...
        local function UpdateDrag(input)
            ...
            CreateTween({Instance, "Position", newPos, 0.1})
        end
        ...
        -- 备用：如果 InputChanged 不触发，也可以用 RunService 监听
...
function redzlib:SetTheme(NewTheme)
	...
	for _, Val in pairs(redzlib.Instances) do
		local ColorProp = GetColor(Val.Instance)
		if not ColorProp then return end
		
		if Val.Type == "Gradient" then
			Val.Instance.Color = Theme["Color Hub 1"]
		elseif Val.Type == "Frame" then
			Val.Instance[ColorProp] = Theme["Color Hub 2"]
		elseif Val.Type == "Stroke" then
			Val.Instance[ColorProp] = Theme["Color Stroke"]
		elseif Val.Type == "Theme" then
			Val.Instance[ColorProp] = Theme["Color Theme"]
		elseif Val.Type == "Text" then
			Val.Instance[ColorProp] = Theme["Color Text"]
		elseif Val.Type == "DarkText" then
			Val.Instance[ColorProp] = Theme["Color Dark Text"]
...
-- 设置窗口缩放
-- 参数 NewScale: 窗口的基准高度（像素），越小窗口越大。范围 300~2000
function redzlib:SetScale(NewScale)
	local Scale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	WindowUIScale = Scale
	WindowScale.Scale = Scale
end

-- 设置悬浮按钮缩放
-- 参数 NewScale: 按钮的基准高度（像素），越小按钮越大。范围 300~2000
function redzlib:SetButtonScale(NewScale)
	local Scale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	ButtonUIScale = Scale
...
function redzlib:MakeWindow(Configs)
	...
	local MainFrame = InsertTheme(Create("ImageButton", WindowLayer, {
	...
	local DropdownHolder = Create("Folder", WindowLayer, {
    	Name = "Dropdown"
    })
	...
	local Minimized, SaveSize, WaitClick  -- 局部变量，供内部函数捕获
	...
	-- 内部保存函数，捕获 Minimized
	local function ConnectSaveInternal(Instance, func)
		Instance.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
	...
	ConnectSaveInternal(ControlSize1, function()
	...
	ConnectSaveInternal(ControlSize2, function()
	...
	function Window:AddMinimizeButton(Configs)
		local Button = MakeDrag(InsertTheme(Create("ImageButton", ButtonLayer, {
            Size = UDim2.fromOffset(50, 50),
            Position = UDim2.fromScale(0.15, 0.15),
            BackgroundTransparency = 1,
            BackgroundColor3 = Theme["Color Hub 2"],
            AutoButtonColor = false
        }), "Frame"), GetButtonScale)
	...
	function Window:Dialog(Configs)
		...
		function Dialog:Button(Configs)
			...
			for _,ChildButton in pairs(ButtonsHolder:GetChildren()) do
				if ChildButton:IsA("TextButton") then
					ChildButton.Size = UDim2.new(1 / ButtonCount, -(((ButtonCount - 1) * 20) / ButtonCount), 0, 32)
				end
			end
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddButton(Configs)
			...
			function Button:Callback(...) return Funcs:InsertCallback(Callback, ...) end
		...
		function Tab:AddToggle(Configs)
			...
			local ToggleCircle = InsertTheme(Create("Frame", Slider, {
			...
			local function SetToggle(Val)
				...
				if Default then
					...
					CreateTween({ToggleCircle, "BackgroundTransparency", 0, 0.25})
					CreateTween({ToggleCircle, "AnchorPoint", Vector2.new(1, 0.5), 0.25, false})
				else
					CreateTween({ToggleCircle, "Position", UDim2.new(0, 0, 0.5), 0.25})
					CreateTween({ToggleCircle, "BackgroundTransparency", 0.8, 0.25})
					CreateTween({ToggleCircle, "AnchorPoint", Vector2.new(0, 0.5), 0.25, false})
				end
				WaitClick = false
			end
			...
			function Toggle:Callback(...) return Funcs:InsertCallback(Callback, ...) end
		...
		function Tab:AddDropdown(Configs)
			...
			local function CalculatePos()
				local currentScale = GetWindowScale()
				local FramePos = SelectedFrame.AbsolutePosition
				local ScreenSize = ScreenGui.AbsoluteSize
				local ClampX = math.clamp((FramePos.X / currentScale), 0, ScreenSize.X / currentScale - DropFrame.Size.X.Offset)
				local ClampY = math.clamp((FramePos.Y / currentScale) , 0, ScreenSize.Y / currentScale)
			...
			-- 下拉框选项管理
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected, OptionOrder do
				local Default = type(OpDefault) ~= "table" and {OpDefault} or OpDefault
				local MultiSelect = DMultiSelect
				local Options = {}
				OptionOrder = {}  -- 记录选项添加顺序
				...
				RemoveOption = function(index, Value)
					...
					if Options[Name] then
						...
						-- 从顺序表中移除
						for i, n in ipairs(OptionOrder) do
							if n == Name then table.remove(OptionOrder, i); break end
						end
				...
				AddNewOptions = function(List, Clear)
					if Clear then
						for _, name in ipairs(OptionOrder) do
							RemoveOption(name)
						end
					end
				...
				for _, item in pairs(DOptions) do
					AddOption(item, item)
				end
				CallbackSelected()
				UpdateSelected()
			...
			function Dropdown:Callback(...) return Funcs:InsertCallback(Callback, ...) end
			...
			function Dropdown:Remove(Option)
				local Options, Order = GetOptions()
				if type(Option) == "number" then
					local name = Order[Option]
					if name then
						RemoveOption(name)
					end
				else
		...
		function Tab:AddSlider(Configs)
			...
			local UIScaleComp = Create("UIScale", LabelVal)
			...
			LabelVal:GetPropertyChangedSignal("Text"):Connect(function()
				UIScaleComp.Scale = 0.3
				CreateTween({UIScaleComp, "Scale", 1.2, 0.1})
				CreateTween({LabelVal, "Rotation", math.random(-1, 1) * 5, 0.15, true})
				CreateTween({UIScaleComp, "Scale", 1, 0.2})
			...
			function SetSlider(NewValue)
				...
				local ClampedValue = math.clamp(NewValue, RawMin, RawMax)
				local SliderPos = (ClampedValue - RawMin) / (RawMax - RawMin)
			...
			function Slider:Callback(...) return Funcs:InsertCallback(Callback, ...) end
